...
/**
 * 积木报表 HTTP 接口数据适配器：
 * 将后端 CommonResult<JmReportServiceByIndustryRespVO> 结构
 * 转换为积木报表可直接识别的「扁平列表」数据集（数组 JSON）。
 *
 * 用法：
 * 1. 在积木报表的数据集配置中选择 HTTP/接口数据源。
 * 2. 填写 URL：/admin-api/credit/jmreport/data/service-by-industry
 * 3. 在「适配器/转换器 Bean 名称」填写：serviceByIndustryParser
 * 4. 数据集无需再配置 data/items 路径，直接按字段名绑定即可。
 *
 * @author Auto
 */
@Component("serviceByIndustryParser")
public class ServiceByIndustryApiDataConvertAdapter implements ApiDataConvertAdapter {
